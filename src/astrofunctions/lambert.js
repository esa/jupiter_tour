...
function lambert_problem(r1, r2, tof, cw)
{
    ...
    // 4 - get solution
	var sol = lambert_3d(r1, r2, tof, lw);
	
    return sol;
}


function lambert_3d(r1, r2, tof, lw)
{
	
    // 1 - Computing non dimensional units
    var R = core.magnitude(r1);
    var V = Math.sqrt(MU_JUP / R);
    var T = R / V;
	
    // 2 - Computing geometry of transfer
    var R2 = core.magnitude(r2);
    var costheta = core.dot(r1, r2) / (R*R2);
    ...
    var out = lambert_2d(s, c, tof/T, lw);
	
    // 4 - Reconstructing the velocity vectors in three dimensions
    var ir1 = core.normalise(r1);
    var ir2 = core.normalise(r2);
	
    // here is the singularity: as when ir1||ir2 this plane is not defined!!
    var ih = (lw ? core.cross(ir2, ir1) : core.cross(ir1, ir2));
    if ((ih[0] + ih[1] + ih[2]) == 0) {
		console.log("lambert is singular in 3D as transfer angle is 180*n degrees\n");
    }
	
    ih = core.normalise(ih);
	
    var it1 = core.cross(ir1, ih);
    var it2 = core.cross(ir2, ih);
...
function lambert_2d(s, c, tof, lw)
{
    var out;
    ...
    ia = Math.log(1 - 0.5);
    ib = Math.log(1 + 0.5);
    out = regula_falsi(ia, ib, new tof_curve(s, c, tof, lw), MAX_ITER, ACCURACY);
    x = Math.exp(out.ia) - 1;
...
function x2tof_partial(s, c, lw, n)
{
    this.s = s;
    this.c = c;
    this.lw = lw;
...
function tof_curve_multi_rev(s, c, tof, lw, n)
{
    this.s = s;
    this.c = c;
    this.tof = tof;
...
function regula_falsi(a, b, F, iterations, accuracy)
{
	var Fa = F.f(a);
	var Fb = F.f(b);
	var Fc = Fa;
